...
class ProjectAnalyzer(ast.NodeVisitor):
    ...
    def print_tree(self, node: Node, indent: str = "", parent_node_count:int=1) -> None:
        self.node_count+=1
        
        rolling_node_count = self.node_count
        if (self.node_count==1):
            nodes.append(StreamlitFlowNode( id='1', 
                            pos=(0, 0), 
            ...
            nodes.append(StreamlitFlowNode(str(self.node_count), 
                    ...
                    'left', 
                    draggable=True))
        edges.append(StreamlitFlowEdge(f'{str(parent_node_count)}-{str(self.node_count)}', str(parent_node_count), str(self.node_count), animated=True, marker_end={'type': 'arrow'}))

        #print(f"{indent}{node.name}")
        for child in node.children:
            self.print_tree(child, indent + "    ",parent_node_count=rolling_node_count )
"""
"""  
...
if __name__ == "__main__":
    project_root = "Test1" #Project Name
    nodes = []
    edges=[]
